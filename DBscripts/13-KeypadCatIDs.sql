...
if not exists (select [Cgr_Key ] from [Categories] where [Cgr_Name] like 'Keypad%')
begin
	INSERT INTO [dbo].[Categories] ([Cgr_Name] ,[Cgr_Parent] ,[Cgr_Visible] ,[Cgr_AppType] ,[Cgr_TxnType])
         VALUES                    ('Keypad' ,null, 0, 2, 0);
    set @catKey = @@IDENTITY;
end;
else
begin
	set @catKey = (select [Cgr_Key ] from [Categories] where [Cgr_Name] like 'Keypad%');
end;
